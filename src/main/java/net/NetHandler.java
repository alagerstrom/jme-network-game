...
public class NetHandler<T> {
    ...
    public NetHandler(int port, Delegate<T> delegate) throws IOException {
        ...
        new AcceptService(serverSocket, this);
    ...
    public interface Delegate<T> {
        void onNewMessage(T message, String senderUniqueName);
        void peerNotResponding(String uniqueName);
        void onNewConnection(String uniqueName);
    }
    ...
    synchronized void handleIncomingMessage(NetMessage<T> netMessage) throws IOException {
        ...
        if (!knownPeers.contains(netMessage.getSender())){
            ...
            if (delegate != null)
                delegate.onNewConnection(netMessage.getSender().getName());
        }
        switch (netMessage.getType()) {
            case MESSAGE:
                if (delegate != null){
                    delegate.onNewMessage(netMessage.getContent(), netMessage.getSender().getName());
                }
                break;
            case HEARTBEAT:
                long now = System.currentTimeMillis();
